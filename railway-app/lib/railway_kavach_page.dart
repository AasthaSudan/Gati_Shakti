...
class _RailwayKavachPageState extends State<RailwayKavachPage> {
  ...
  // Removed _fetchAlerts, now using stream
  ...
  Widget build(BuildContext context) {
    return Theme(
      data: ThemeData.dark().copyWith(
        ...
        appBarTheme: AppBarTheme(backgroundColor: _cardColor, elevation: 0),
      ...
      child: Scaffold(
        ...
        body: StreamBuilder<List<AlertModel>>(
          ...
          builder: (context, snapshot) {
            ...
            // Find new alerts that were not present before
            final newCurrentAlerts =
                alerts.where((alert) => !alert.isProcessed).toList();
            final previousAlertIds = _currentAlerts.map((a) => a.id).toSet();
            final newAlerts =
                newCurrentAlerts
                    .where((a) => !previousAlertIds.contains(a.id))
                    .toList();
            // Show notification for each new alert
            for (final alert in newAlerts) {
              _showNotification(alert);
            }
            _currentAlerts = newCurrentAlerts;
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        children: [
                          Icon(Icons.train, color: _primaryColor, size: 24),
    ...
    if (_pastAlerts.isEmpty) {
      return Center(
        child: Column(
          mainAxisAlignment: MainAxisAlignment.center,
          children: [
            Icon(Icons.history, size: 64, color: _secondaryTextColor),
            const SizedBox(height: 16),
            Text(
              'No past alerts',
              style: TextStyle(fontSize: 18, color: _secondaryTextColor),
              ...
              ListTile(
                ...
                subtitle: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 4),
                    Text(
                      'Status: ${alert.status.replaceAll('_', ' ').capitalize()}',
                    ),
                  ],
                ),
                trailing:
                            ...
                            child: ClipRRect(
                              ...
                              child: Image.network(
                                ...
                                errorBuilder: (context, error, stackTrace) {
                                  return Center(
                                    child: Icon(
                                      alert.animalType == 'elephant'
                                          ? Icons.pets
                                          : alert.animalType == 'deer'
                                          ? Icons.nature
                                          : Icons.broken_image,
                                      size: 40,
                                      color: Colors.grey,
